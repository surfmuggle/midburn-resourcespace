...
$step_1_of_3 = $global_edit_then_upload_then_edit && !$upload_review_mode && !empty($uploader);
$step_1_of_3 = false;
echo "collection_add: $collection_add .";
print_r($_REQUEST);
//echo "save_auto_next".$save_auto_next.($save_auto_next ? "Y":"N"); //AAA
//echo 'upload_review_mode:'.$upload_review_mode.", ";
//print_r($_REQUEST);
//$save_auto_next = true; //AAA

$uploadparams = array();
$uploadparams["relateto"] = getval("relateto","");
$uploadparams["redirecturl"] =  getval("redirecturl","");
if (!$step_1_of_3) $uploadparams["collection_add"] =  $collection_add;
...
function SaveAndClearButtons($extraclass="")
   ...
   if($clearbutton_on_edit)
    ...
    <input <?php if ($multiple) { ?>onclick="return confirm('<?php echo $lang["confirmeditall"]?>');"<?php } ?> name="save" class="editsave" type="submit" value="&nbsp;&nbsp;<?php echo ($ref>0)?(($upload_review_mode || $show_next_button_on_edit)?$lang["saveandnext"]:$lang["save"]):$lang["next"]?>&nbsp;&nbsp;" />
    <?php
...
<form method="post"
      ...
      onsubmit="
        preventautosave = true;
        ok = <?php echo ($modal ? 'Modal' : 'CentralSpace'); ?>Post(this, true);
<?php
if ($step_1_of_3)
{
?>
    if (true || ok) {
        mainform = jQuery('#mainform');
        mainform.attr('action', mainform.attr('action') + '&collection_add='+ jQuery('#collection_add').val());
        alert(jQuery('#collection_add').val() +', '+mainform.attr('action') );
    }
<?php
}
?>
       ...
       <?php if ($check_edit_checksums)
        ...
        if('' == getval('submitted', ''))
            ...
            if(!isset($selected_type))
                {
                if (!isset($shown_resource_types[0])) 
                    {
                        trigger_error("You do not have permissions to upload any of the available resource types", E_USER_ERROR);
                    }
   ...
   {?>
    ...
    # Should this field be displayed?
        ...
        # draw new tab panel?
            ...
            //here we print the user defined metadata fields
			node_field_options_override($fields[$n]);
            if ($step_1_of_3) {
              if (in_array($n, array(0,1))) continue; //TBD this is a workaround, not generic. instead add a field to the metadata field definition
            }
