        ...
        if (($save_errors === true || $is_template) && getval("tweak","")=="")
            ...
            if ($ref > 0 && getval("save","") != "")
                ...
                if (!hook('redirectaftersave') && !$modal)
                  {
                  if (empty($show_next_button_on_edit)) 
                    redirect(generateURL($baseurl_short . "pages/view.php",$urlparams, array("refreshcollectionframe"=>"true")));
                  else 
                    redirect(generateURL($baseurl_short . "pages/edit.php?search=%21last1000&offset=0&order_by=resourceid&sort=DESC&archive=0&ref=14329",array(), array("refreshcollectionframe"=>"true")));
                  }
                }
            else
                {
...
function ShowHelp(field)
{
 ...
 jQuery(document).ready(function() {
    ...
    jQuery("#collection_add option[value='new']").remove();
    jQuery("#collection_add option[value='false']").remove();
    jQuery('#collection_add').val(-224);
...
function EditNav() # Create a function so this can be repeated at the end of the form also.
    {
    global $baseurl_short,$ref,$search,$offset,$order_by,$sort,$archive,$lang,$modal,$restypes,$disablenavlinks,$upload_review_mode, $urlparams, $fancy_edit_nav_button;
    ?>
    <div class="<?php echo (empty($fancy_edit_nav_button)) ? "backtoresults":"QuestionSubmit NoPaddingSaveClear";?>" <?php if (!empty($fancy_edit_nav_button)) echo 'style="border-bottom: 1px lightgrey solid;"';?> > 
    
    <?php
    if(!$disablenavlinks && !$upload_review_mode)
        {
        $button_style =  (empty($fancy_edit_nav_button)) ? '' : 'style="height: auto;padding: 5px;box-shadow: 1px 1px 8px -2px;border: 0;border-radius: 5px;"';
    ?>
        <a <?php echo $button_style; ?> class="prevLink fa fa-arrow-left" onClick="return <?php echo ($modal?"Modal":"CentralSpace") ?>Load(this,true);" href="<?php echo generateURL($baseurl_short . "pages/edit.php",$urlparams, array("go"=>"previous")); ?>">Prev</a>
    <?php if (empty($fancy_edit_nav_button))
    {?>
    ...
    ?>
        <a <?php echo $button_style; ?> class="nextLink fa fa-arrow-right" onClick="return <?php echo ($modal?"Modal":"CentralSpace") ?>Load(this,true);" href="<?php echo generateURL($baseurl_short . "pages/edit.php",$urlparams, array("go"=>"next")); ?>">Next</a>
...
function SaveAndClearButtons($extraclass="")
   ...
   global $show_next_button_on_edit;
    $save_btn_value = ($ref > 0 ? (($upload_review_mode || $show_next_button_on_edit) ? $lang["saveandnext"] : $lang["save"]) : $lang["next"]);
    ...
    <?php
    global $disable_auto_next;
    if ($upload_review_mode && empty($disable_auto_next))
        {
...
<input type="hidden" name="upload_review_mode" value="<?php echo ($upload_review_mode?"true":"")?>" />
   <div class="BasicsBox" style="<?php echo empty($uploader) ? "" : "display:none";?>"> <!-- changed by Avner: hide this header -->
   ...
   elseif ($ref>0)
      ...
      if (!hook("replaceeditheader")) 
         ...
         if (empty($fancy_edit_nav_button))
         if (!$upload_review_mode) { ?>
        ...
        <?php } ?>
         ...
         <?php
         if ($edit_show_save_clear_buttons_at_top || $upload_review_mode || $show_next_button_on_edit) { SaveAndClearButtons("NoPaddingSaveClear");}
      ...
      if((!$is_template && !checkperm('F*')) || $custompermshowfile)
         ...
         <div class="Question" id="question_file">
            <label>
            <?php 
            if (empty($hide_file_label_on_upload)) 
         ...
         if ($resource["has_image"]==1)            
            { 
                $max_height_style = empty($max_image_preview_height) ? "" :  ("max-height:".$max_image_preview_height."px;");
                $top_margin = empty($upload_review_mode) ? "" : "margin-top: -75px;";
            ?>
            <img id="preview" align="top" src="<?php echo get_resource_path($ref,false,($edit_large_preview && !$modal?"pre":"thm"),false,$resource["preview_extension"],-1,1,false)?>" class="ImageBorder" style="margin-right:10px;<?php echo $max_height_style.$top_margin; ?>"/>
...
?><div <?php if($collapsible_sections){echo'class="CollapsibleSection"';}?> id="ResourceMetadataSection<?php if ($ref<0) echo "Upload"; ?>">
    
<?php if (!empty($uploader)) 
{
?>
<h1>Upload Photos</h1>
The following settings are common to all photos you're about to upload. You will be able to change them individually later.
<?php
}
}
...
function SelectTab(tab)
{
    ...
    # Should this field be displayed?
        ...
        # draw new tab panel?
			...
			$hack_hide_some_fields = true;
            if ($hack_hide_some_fields) {
              if (in_array($n, array(1))) continue; //TBD this is a workaround, not generic. instead add a field to the metadata field definition
            }			
...
<?php 
if (count($shown_resource_types)<=1) echo "<script>document.getElementById('question_resourcetype').style.display='none';</script>";    
?>    
